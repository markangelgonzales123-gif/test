...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pds_data'])) {
    ...
    foreach ($dynamic_sections as $section) {
        if (isset($pds_data[$section]) && is_array($pds_data[$section])) {
            ...
            // --- UPDATED LOGIC FOR ROBUST FILTERING ---
            $filtered_rows = [];
            foreach ($pds_data[$section] as $row) {
                // Check if the current row (which is an array) has any non-empty value
                $has_data = false;
                if (is_array($row)) {
                ...
                if ($has_data) {
                    $filtered_rows[] = $row; // Keep rows that have data
                }
            }
            // Re-index numerically
            $pds_data[$section] = array_values($filtered_rows);
            // --- END UPDATED LOGIC ---
...
<script>
    ...
    // FIX FOR SYNTAX ERROR: Output the PHP JSON data directly as a JavaScript object/value.
    const pdsData = <?php echo $pds_data_json; ?>; 
    ...
    // HTML Templates for dynamic rows (Confirmed these names match the JSON structure)
    const templates = {
        ...
        'learning_dev_table': (index) => `
            ...
            <td class="text-center"><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)"><i class="bi bi-x"></i></button></td>
        `,
        // The single-column templates are now confirmed to be correct in their naming
    ...
    function addRow(tableId, data = {}) {
        ...
        if (Object.keys(data).length > 0) {
            // Use a short delay to ensure elements are added to the DOM before querying/setting values
            setTimeout(() => {
                // Select all inputs, textareas, and selects that belong to this newly created, indexed row
                // We use a more generic query on the row itself
                const inputs = newRow.querySelectorAll(`input, select, textarea`);
                
                inputs.forEach(input => {
                    // Extract the field name from the full name attribute (e.g., gets 'school' from 'pds_data[educational_background][0][school]')
    ...
    /**
     * Toggles visibility and required attribute for conditional fields (Q34-Q42).
     */
    ...
    function loadPDS() {
        ...
        allSections.static.forEach(sectionKey => {
            ...
            if (sectionKey === 'references') {
                // Special handling for the fixed 3 references
                for (let i = 1; i <= 3; i++) {
                    // References are indexed as strings '1', '2', '3' in the JSON
                    const refData = sectionData[i.toString()]; 
                    if (refData) {
                        for (const subField of ['name', 'address', 'tel']) {
                            const inputName = `pds_data[references][${i}][${subField}]`;
                            const inputElement = document.querySelector(`[name="${inputName}"]`);
                            if (inputElement) {
                                inputElement.value = refData[subField] || '';
                            }
                        }
                    }
                }
            ...
            // Generic logic for personal_info, family_background, conditional
            for (const fieldKey in sectionData) {
                // This name targets the main input (e.g., pds_data[personal_info][surname])
                const inputName = `pds_data[${sectionKey}][${fieldKey}]`;
                
                // This name targets the details field (e.g., pds_data[conditional][q34_details])
                const detailsName = `${fieldKey}_details`;
                
                // Use querySelectorAll to find all elements with the same name (needed for radio buttons)
                const inputElements = document.querySelectorAll(`[name="${inputName}"]`);
                
                if (inputElements.length > 0) {
                    const firstInput = inputElements[0];
                    const valueToSet = sectionData[fieldKey]; // The saved value

                    if (firstInput.type === 'radio') {
                            // Handle radio buttons
                        
                        inputElements.forEach(radioInput => {
                            if (radioInput.value === valueToSet) {
                                radioInput.checked = true;
                                
                                // Special handling for conditional questions to show/hide details
                                if (sectionKey === 'conditional') {
                                    // The fieldKey is already the question base name (e.g., 'q34', 'q35a')
                                    handleConditional(fieldKey, radioInput.value);
                                    
                                    // Also manually set the details field if it exists in the data
                                    const detailElement = document.querySelector(`[name="pds_data[${sectionKey}][${detailsName}]"]`);
                                    if (detailElement) {
                                        detailElement.value = sectionData[detailsName] || '';
                                    }
                                }
                            }
                        });
                    } else {
                        // Handle text, date, textarea, select, and other inputs
                        firstInput.value = valueToSet || '';
                    }
                }
            }
        });
